...
def is_version_compatible(version: str) -> bool:
    ...
    return parse_version(version) >= parse_version(
        constants.MINIMUM_SUPPORTED_DOMINO_VERSION
    )
...
def is_cluster_type_supported(version: str, cluster_type: str) -> bool:
    ...
    return next(
        (
            True
            for ct, min_version in constants.CLUSTER_TYPE_MIN_SUPPORT
            if ct == cluster_type and curr_version >= parse_version(min_version)
        ),
        False,
    )
...
def is_compute_cluster_properties_supported(version: str) -> bool:
    return parse_version(version) >= parse_version(
        constants.MINIMUM_DISTRIBUTED_CLUSTER_SUPPORT_DOMINO_VERSION
    )


def is_on_demand_spark_cluster_supported(version: str) -> bool:
    return parse_version(version) >= parse_version(
        constants.MINIMUM_ON_DEMAND_SPARK_CLUSTER_SUPPORT_DOMINO_VERSION
    )


def is_external_volume_mounts_supported(version: str) -> bool:
    return parse_version(version) >= parse_version(
        constants.MINIMUM_EXTERNAL_VOLUME_MOUNTS_SUPPORT_DOMINO_VERSION
    )
...
def domino_is_reachable(url=os.getenv(constants.DOMINO_HOST_KEY_NAME), port="443"):
    """
