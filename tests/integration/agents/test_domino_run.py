...
def test_domino_run_dev(setup_mlflow_tracking_server, mocker, mlflow, tracing, logging):
    ...
    @tracing.add_tracing(name="add_numbers", autolog_frameworks=['sklearn'], evaluator=lambda span: {'add_numbers': span.outputs})
    def add_numbers(x, y):
        return x + y
...
def test_domino_run_configure_experiment_name(setup_mlflow_tracking_server, mlflow, logging, tracing):
    ...
    traces = mlflow.search_traces(experiment_ids=[exp_id], filter_string="trace.name = 'unit'")
