...
def test_domino_run_dev(setup_mlflow_tracking_server, mocker, mlflow, tracing, logging):
    ...
    @tracing.add_tracing(
        ...
        evaluator=lambda span: {"add_numbers": span.outputs},
    )
    def add_numbers(x, y):
        return x + y
    ...
    assert (
        create_external_model_spy.call_count == 1
    ), "create external model should be called once"
    ...
    ts = mlflow.search_traces(
        experiment_ids=[exp.experiment_id], model_id=model.model_id, return_type="list"
    )
    ...
    assert (
        run.data.tags.get(AGENT_RUN_TAG) == "false"
    ), "DominoRun should tag the run as not an agent run"

    # verify run has summary metrics logged to it
    # average of outputs is 2 + 4/2 = 3
    assert (
        run.data.metrics["mean_add_numbers"] == 3
    ), "average of add_numbers should be 3"


def test_domino_run_dev_custom_aggregator(
    setup_mlflow_tracking_server, mlflow, tracing, logging
):
    """
    DominoRun will contain custom summarizaiton metrics for eval traces
    """
    mlflow.set_experiment("test_domino_run_custom_aggregator")

    @tracing.add_tracing(name="median", evaluator=lambda span: {"median": span.outputs})
    def for_median(x):
        return x

    @tracing.add_tracing(name="mean", evaluator=lambda span: {"mean": span.outputs})
    def for_mean(x):
        return x

    @tracing.add_tracing(name="stdev", evaluator=lambda span: {"stdev": span.outputs})
    def for_stdev(x):
        return x

    @tracing.add_tracing(name="min", evaluator=lambda span: {"min": span.outputs})
    def for_min(x):
        return x

    @tracing.add_tracing(name="max", evaluator=lambda span: {"max": span.outputs})
    def for_max(x):
        return x
...
def test_domino_run_dev_bad_custom_aggregator(
    setup_mlflow_tracking_server, mlflow, tracing, logging
):
    """
    DominoRun will fail if one of the aggregators is invalid
    """
    mlflow.set_experiment("test_domino_run_dev_bad_custom_aggregator")
...
def test_domino_run_configure_experiment_name(
    setup_mlflow_tracking_server, mlflow, logging, tracing
):
    """
    ...
    traces = mlflow.search_traces(
        experiment_ids=[exp_id], filter_string="trace.name = 'unit'"
    )

    assert (
        run.info.experiment_id == exp_id
    ), "run should belong to test_domino_run_configure_experiment_name_other"
    assert len(traces) == 1


def test_domino_run_extend_current_run(
    setup_mlflow_tracking_server, mlflow, logging, tracing
):
    """
    ...
    @tracing.add_tracing(name="unit", evaluator=lambda span: {"unit": span.outputs})
    def unit(x):
        return x
    ...
    traces = mlflow.search_traces(
        ...
        filter_string=f"metadata.mlflow.sourceRun = '{first_run_id}'",
        return_type="list",
    )
    ...
    assert (
        resumed_run.data.tags.get(AGENT_RUN_TAG) == "false"
    ), "DominoRun should tag the run as not an agent run"
...
def test_domino_run_should_not_swallow_exceptions(
    setup_mlflow_tracking_server, mlflow, logging
):
    """
    ...
    with pytest.raises(ZeroDivisionError):
        with logging.DominoRun() as _run:  # noqa: F841
            1 / 0


def test_domino_run_parallelized_logic(
    setup_mlflow_tracking_server, mlflow, logging, tracing
):
    """
    ...
    assert get_run_id(traces_a[0]) == get_run_id(
        traces_b[0]
    ), "The a and b traces should belong to the same run"
...
def test_domino_run_extend_concluded_run_manual_evals_mean_logged(
    ...
    mlflow.set_experiment(
        "test_domino_run_extend_concluded_run_manual_evals_mean_logged"
    )
    ...
    with logging.DominoRun(run_id=concluded_run_id):
        traces_resp = tracing.search_traces(
            run_id=concluded_run_id, trace_name="add_numbers"
        )
    ...
    # average of 2 + 4 = 3
    assert (
        run.data.metrics["mean_helpfulness"] == 3
    ), "average of helpfulness should be 3"


def test_domino_run_extend_concluded_run_manual_evals_custom_aggregator_logged(
    setup_mlflow_tracking_server, mlflow, tracing, logging
):
    """
    ...
    """
    mlflow.set_experiment(
        "test_domino_run_extend_concluded_run_manual_evals_custom_aggregator_logged"
    )
    ...
    with logging.DominoRun(
        run_id=concluded_run_id, custom_summary_metrics=custom_summary_metrics
    ):
        traces_resp = tracing.search_traces(
            run_id=concluded_run_id, trace_name="add_numbers"
        )
...
def test_domino_run_recomputes_existing_aggregations(
    setup_mlflow_tracking_server, mlflow, tracing, logging
):
    """
    ...
    @tracing.add_tracing(name="agg", evaluator=lambda span: {"agg": span.outputs})
    def agg_fn(x):
        return x
    ...
    with logging.DominoRun(
        custom_summary_metrics=[("agg", "mean"), ("agg", "max")]
    ) as run:
        run_id = run.info.run_id
        agg_fn(1)
        agg_fn(3)
    ...
    with logging.DominoRun(run_id=run_id) as _run2:  # noqa: F841
        agg_fn(5)
...
def test_domino_agent_context_tags_run(setup_mlflow_tracking_server, mlflow, logging):
    ...
    run = mlflow.get_run(run_id)
    assert (
        run.data.tags.get(AGENT_RUN_TAG) == "true"
    ), "DominoAgentContext should tag the run"


def test_domino_agent_context_tags_resumed_run(
    setup_mlflow_tracking_server, mlflow, logging
):
    ...
    assert (
        run.data.tags.get(AGENT_RUN_TAG) == "true"
    ), "DominoAgentContext should tag the resumed run"
