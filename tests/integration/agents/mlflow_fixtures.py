...
def add_prod_tags(traces: Optional[list], agent_id: str, agent_version: str):
    ...
    for t in traces:
        ...
        client.set_trace_tag(
            t.info.trace_id, "mlflow.domino.app_version_id", agent_version
        )
...
def create_span_at_time(name: str, inputs: int, hours_ago: int, experiment_id: str):
    ...
    span = mlflow.start_span_no_context(
        name=name, inputs=inputs, experiment_id=experiment_id, start_time_ns=ns
    )
    span.end()
...
def fixture_create_prod_traces(
    ...
    env_vars = TEST_AGENTS_ENV_VARS | {
        ...
        "DOMINO_APP_ID": agent_id,
    }
    with patch.dict(os.environ, env_vars, clear=True):
        tracing.init_tracing()
        if hours_ago is not None:
            experiment = mlflow.get_experiment_by_name(
                build_agent_experiment_name(agent_id)
            )
            create_span_at_time(
                trace_name, hours_ago, hours_ago, experiment.experiment_id
            )
        else:
            one(1)
        ...
        ts = mlflow.search_traces(
            ...
            filter_string=f"trace.name = '{trace_name}'",
            return_type="list",
        )
