...
def download_results(domino, commitId, results, output, exp):
    ...
    for file in files["data"]:
        pattern = None
        if results is None and exp is not None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="url",
        default=URL,
        help="Your domino url",
        required=True,
    )
    parser.add_argument(
        "-f", "--file", dest="file", help="File to upload", required=True
    )
    parser.add_argument(
        "-p",
        "--project",
    ...
    parser.add_argument(
        ...
        dest="dest",
        default=UPLOAD_DIR,
        help="Upload destination with the project",
    )
    parser.add_argument(
        "-r", "--run", dest="run", default=RUN, help="Batch process to run"
    )
    parser.add_argument(
        "-R",
        "--results",
        dest="results",
        help="The results to download",
        required=False,
    )
    parser.add_argument(
        "-e",
    ...
    parser.add_argument(
        ...
        "--output",
        dest="output",
        help="The output of the results (default results)",
        required=False,
    )
    parser.add_argument(
        "-i",
        ...
        dest="interval",
        default=1,
        help="Poll interval for the run status",
    )
    ...
    if args.results is not None or args.expression is not None:
